  ...
  if (lessonPageDate) {
    ...
    const last5Dates = (!grades || !Array.isArray(grades)) ? [] : (() => {
      const datesSet = new Set(
        grades
          .filter(g => g.subject === selectedSubject && g.date !== lessonPageDate)
          .map(g => g.date)
      );
      return Array.from(datesSet).sort((a, b) => b.localeCompare(a)).slice(0, 5);
    })();
                ...
                {lpStudentGrades.map((s, idx) => {
                  ...
                  return (
                    <tr key={s.id} className="border-b border-gray-100 hover:bg-white/40 transition-colors">
                      ...
                      {last5Dates.map(d => {
                        const g = grades.find(grade => grade.studentId === s.id && grade.date === d && grade.subject === selectedSubject);
                        return (
...
const TestResultsSection: React.FC<{
  ...
  // Пересчитать результаты на основе вручной отмеченных ответов
